  ...
  return (
    <Screen>
      ...
      <ScrollView keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor={colors.neon} />} contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
        ...
        {activities.map((activity) => <FeedCard key={activity.id} activity={activity} onReact={react} />)}
      </ScrollView>
    </Screen>
  );
}

function FeedCard({ activity, onReact }: { activity: Activity; onReact: (activityId: string, type: string) => void }) {
  return (
    <Card>
      <View style={{ flexDirection: "row", gap: 12 }}>
        ...
        <View style={{ flex: 1, gap: 6 }}>
          ...
          {activity.drinkLog ? (
            <View style={{ backgroundColor: "rgba(255,255,255,0.06)", padding: 10, borderRadius: 14 }}>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        {reactions.map(([type, label]) => {
          ...
          return <Text key={type} onPress={() => onReact(activity.id, type)} style={{ color: colors.text, backgroundColor: "rgba(255,255,255,0.1)", paddingHorizontal: 10, paddingVertical: 8, borderRadius: 999, fontWeight: "800" }}>{label} {count || ""}</Text>;
        })}
      </View>
      <Muted>Comment soon. The group chat is training.</Muted>
    </Card>
  );
}
